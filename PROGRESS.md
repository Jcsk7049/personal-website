...
- **🟡 09-25 首頁技能方塊與幾何漸變**：技能區依參考圖改成四張分類卡、卡內每項技能獨立成小方塊；首頁左側加低彩度鈦灰幾何漸變，與白黑基底一致。首頁副標為 `develop.ing`、鈦金屬灰 `#737C84`。Production build 通過；尚待 commit/push 與 Cloudflare 部署核對。

- **🟡 09-25 技能區滿版高度**：桌機技能欄延展至 Hero 可用的整個高度，四張卡等高填滿；卡片和技能方塊保留固定 gap，手機自然堆疊。待 build、commit/push、部署確認。
