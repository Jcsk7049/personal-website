...
- **🟡 09-25 經歷／學歷左右留白**：依截圖把 Experience 內容容器由 `px-6 md:px-10` 微調為 `px-8 md:px-14`，左右兩側各向內收 8px（手機）／16px（桌機）。`npm.cmd run build` 成功；待 commit/push。
...
- 依本人截圖把 Experience 內容容器改為 `px-8 md:px-14`，兩側向內收窄，經歷與學歷同步對齊。
- `npm.cmd run build` 與 `git diff --check` 通過；待 commit/push。
