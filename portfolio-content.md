...
| 機器學習 | 進階 | XGBoost、LightGBM Ensemble 二元分類，結合 SHAP 值實現模型可解釋性，有效處理嚴重類別不平衡的金融詐欺場景。|
...
**ID**: aws-hackathon  
**期間**: 2026  
**Tags**: XGBoost、LightGBM、AWS SageMaker、AWS Glue、SHAP、Streamlit  
**GitHub**: https://github.com/Jcsk7049/bitoguard-aml  
**Demo**: https://bitoguard-aml-kykw6kfawalbeq8vjy6yv8.streamlit.app/

**一句話描述**：
XGBoost / LightGBM × S3 → Glue → Athena → SageMaker → Lambda → Bedrock 六服務端對端 AML 管線；63,770 帳戶 / 413,235 筆交易，31 項行為特徵（含圖論黑名單鄰居）+ SHAP 合規報告識別人頭帳戶，Streamlit 儀表板已部署上線。
...
**技術架構**：
S3 → Glue ETL → Athena SQL → SageMaker（XGBoost 訓練 depth=8, lr=0.05, 1000 rounds）→ Lambda（事件驅動推論）→ Bedrock（SHAP 可解釋性報告）→ Streamlit 儀表板

**特徵工程**：31 項行為指標，重要性前三：total_volume、age（帳戶年齡）、swap_twd_volume

**模型效能**（5-fold Stratified CV）：
- Precision：9.83%（± 8.12%）；Recall：0.43%（± 0.36%）
- 競賽提交保守閾值（0.65），刻意偏向 Precision 避免誤判
