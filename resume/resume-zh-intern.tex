...
\usepackage{fontspec}
\usepackage{xeCJK}
% 以家族名靠系統字型庫解析（本機 Windows 實裝 Noto Sans TC）。
% AutoFakeBold：Windows 版 Noto Sans TC 沒有獨立 Bold 面，靠演算法加粗
% 讓 \textbf/標題/姓名的中文真的變粗（否則會默默退回一般字重）。不依賴系統字重、可攜。
% 換到 Overleaf 若報 font not found，改成 Noto Sans CJK TC 再試。
\setCJKmainfont{Noto Sans TC}[AutoFakeBold=2.5]
% Latin Modern is not installed as a Windows system font in the bundled TinyTeX
% runtime. Arial is present on Windows and keeps XeLaTeX builds reproducible.
\setmainfont{Arial}
...
\newcommand{\resumeItemListStart}{\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=4pt]}
...
\section{技術能力}
 \begin{itemize}[leftmargin=0.0in, label={}]
    \small{\item{
     ...
     \textbf{機器學習}{: TensorFlow/Keras, LSTM/時間序列, Integrated Gradients/XAI, SHAP} \\
